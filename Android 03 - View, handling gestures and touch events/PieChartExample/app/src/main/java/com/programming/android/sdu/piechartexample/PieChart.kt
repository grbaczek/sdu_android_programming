...
class PieChart : ViewGroup {
    ...
    private fun onDataChanged() {
        ...
        for (it in mData) {
            ...
            it.mShader = SweepGradient(
                    ...
                    it.mHighlight,
                    it.mColor,
                    it.mColor), floatArrayOf(0f,
                    (360 - it.mEndAngle).toFloat() / 360.0f,
                    (360 - it.mStartAngle).toFloat() / 360.0f,
                    1.0f
            ))
        }
        calcCurrentItem()
    ...
    private fun init() {
        ...
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            mAutoCenterAnimator?.addListener(object : AnimatorListener {
        ...
        mScroller = if (Build.VERSION.SDK_INT < 11) {
            Scroller(context)
        } else {
            Scroller(context, null, true)
        }
        ...
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            mScrollAnimator?.addUpdateListener(AnimatorUpdateListener { tickScrollAnimation() })
        }
    ...
    private val isAnimationRunning: Boolean
        private get() = !mScroller!!.isFinished || Build.VERSION.SDK_INT >= 11 && mAutoCenterAnimator!!.isRunning
